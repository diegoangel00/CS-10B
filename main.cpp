...
void noNegatives(int* x) {
    //checks to see if the input is negative given its address
    //changes value to 0 if negative
    if (x[0] < 0)
        x[0] = 0;
}

void swap(int* x, int* y) {
    //accepts 2 memory addresses
    // swaps values at memory addresses
    // doesn't return value
 
    // temp variable to store first input
    int temp;
    temp = *x;
    *x = *y;
    *y = temp;
